...
func resetElectionTimeout() (int){
	...
	electionTimeout = 15 + rand.Intn(30-15+1)
...
func handleMessage(message string, conn net.Conn) {
	...
	// If our term is newer than the message sender's, tell them they are behind.
	if (term > msgTerm) { 
	...
	// Only do stuff if we are in the same term as the sender.
	if (msgTerm == term) {
		if (state == "follower") {
			if (directive == "leaderHeartbeat") { // If we hear the leader's heartbeat
				...
				electionTimeout = resetElectionTimeout() // Otherwise, reset our election timeout
			}
